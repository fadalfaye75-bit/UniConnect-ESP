        ...
        {displayedAnnouncements.map((ann, index) => {
          ...
          return (
            <div 
              ...
              className={`group relative rounded-[2.5rem] border p-8 transition-all duration-500 flex flex-col gap-8 hover:-translate-y-1 hover:shadow-2xl animate-fade-in ${
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-3 mb-4">
                    ...
                    <div className="ml-auto flex items-center gap-3">
                       ...
                       {canDelete && (
                         <button onClick={(e) => handleDelete(e, ann.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl opacity-0 group-hover:opacity-100"><Trash2 size={16} /></button>
                       )}
                    </div>
                  </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Diffuser une annonce">
        <form onSubmit={handleCreateSubmit} className="space-y-6">
          ...
          <div>
            ...
            <textarea required rows={4} value={newAnn.content} onChange={e => setNewAnn({...newAnn, content: e.target.value})} className="w-full px-5 py-3.5 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 font-bold outline-none italic" placeholder="Détails du message..." />
          </div>
          
          <div className="grid grid-cols-2 gap-4">
             <div>
                <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Priorité</label>
                <select value={newAnn.priority} onChange={e => setNewAnn({...newAnn, priority: e.target.value as any})} className="w-full px-5 py-3 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-xs font-black uppercase outline-none">
                   <option value="normal">Normal</option>
                   <option value="important">Important</option>
                   <option value="urgent">Urgent</option>
                </select>
             ...
             <div>
                ...
                <select value={newAnn.className} onChange={e => setNewAnn({...newAnn, className: e.target.value})} className="w-full px-5 py-3 rounded-2xl bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 text-xs font-black uppercase outline-none">
                   <option value="Général">Général</option>
                   {classes.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                </select>
             </div>
